...
@J2clTestInput(ByteBufferTest.class)
public class ByteBufferTest extends AbstractBufferTest {
  ...
  private void assertContentEquals(ByteBuffer buf, byte array[], int offset, int length) {
  ...
  private void assertContentLikeTestData1(
      ByteBuffer buf, int startIndex, byte startValue, int length) {
  ...
  public void testPutChar() {
    ...
    for (int i = 0; buf.remaining() >= nbytes; i++) {
      ...
      buf.reset();
      buf.get(bytes);
      assertTrue("Wrong value at " + i, Arrays.equals(char2bytes(value, buf.order()), bytes));
    }
  ...
  */
  ...
  public void testPutInt() {
    ...
    for (int i = 0; buf.remaining() >= nbytes; i++) {
      ...
      buf.reset();
      buf.get(bytes);
      assertTrue("Wrong value at " + i, Arrays.equals(int2bytes(value, buf.order()), bytes));
    }
  ...
  public void testPutLong() {
    ...
    for (int i = 0; buf.remaining() >= nbytes; i++) {
      ...
      buf.reset();
      buf.get(bytes);
      assertTrue("Wrong value at " + i, Arrays.equals(long2bytes(value, buf.order()), bytes));
    }
  ...
  public void testPutShort() {
    ...
    for (int i = 0; buf.remaining() >= nbytes; i++) {
      ...
      buf.reset();
      buf.get(bytes);
      assertTrue("Wrong value at " + i, Arrays.equals(short2bytes(value, buf.order()), bytes));
    }
